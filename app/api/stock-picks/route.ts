...
import { getKrValueUniverse, type ValueRow } from "@/lib/naver";
...
function screen(uni: ValueRow[], limit: number): ValueRow[] {
  return uni
    .filter(
      (s) =>
        s.per !== null &&
        ...
        s.roe !== null &&
        s.roe >= 8 && // 최소한의 수익성
        s.marketCap >= 3000, // 시총 3000억 이상 (초소형주 노이즈 제거)
    )
...
function buildPrompt(s: ValueRow, news: Article[]): string {
...
[지표] PER ${s.per} · ROE ${s.roe}% · 시가총액 ${s.marketCap}억원 · 현재가 ${s.price ?? "?"} · 등락 ${s.changeRate ?? "?"}%
...
export async function POST(req: NextRequest) {
  ...
  // 1) 정량 스크리닝
  const uni = await getKrValueUniverse(2);
  ...
  return NextResponse.json({
    ...
    picks,
    screened: uni.length,
    criteria: "PER 40배 미만 · ROE 8% 이상 · 시총 3000억 이상 → ROE/PER 상위",
