...
export async function DELETE(
  ...
  const { error } = await db.from("news_comments").delete().eq("id", id);
  if (error) {
    return NextResponse.json({ ok: false, reason: error.message }, { status: 500 });
  }
  await db.rpc("bump_news_comments", { p_url: String(comment.article_url), d: -1 });
